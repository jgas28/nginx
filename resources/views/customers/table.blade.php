    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                <span>Show</span>
                <select id="customers-per-page" class="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
            ...
            @forelse($customers as $customer)
                <tr class="hover:bg-slate-50/80">
                    <td class="px-4 py-3.5 text-sm text-slate-500 sm:px-6">
                        {{ ($customers->firstItem() ?? 0) + $loop->index }}
                    </td>
                    <td class="px-4 py-3.5 font-semibold text-slate-900 sm:px-6">
                        <div class="inline-flex items-center gap-2">
                            <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-r from-blue-50 to-cyan-100 text-blue-600 ring-1 ring-blue-100">
            ...
            @empty
                <tr>
                    <td colspan="3" class="px-4 py-12 text-center text-slate-500 sm:px-6">
                        ...
                        <p class="mt-3 font-medium text-slate-700">No customers found</p>
                        <p class="mt-1 text-sm">Try a different search term or add a new customer.</p>
                    </td>
                </tr>
            @endforelse
        </tbody>
...
<div class="flex flex-col gap-3 border-t border-slate-200 px-4 py-4 text-sm text-slate-500 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
    <p>
        Showing
        <span class="font-semibold text-slate-700">{{ $customers->firstItem() ?? 0 }}</span>
        to
        <span class="font-semibold text-slate-700">{{ $customers->lastItem() ?? 0 }}</span>
        of
