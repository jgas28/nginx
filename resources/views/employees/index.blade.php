...
@section('content')
    <!-- Page Header -->
    <h1 class="text-2xl font-semibold mb-4 text-gray-700">Employee</h1>

    <!-- Container to hold both elements in a row -->
    <div class="flex justify-between items-center mb-4">
        <!-- Search Form (aligned to the left) -->
        <form method="GET" action="{{ route('employees.index') }}" class="flex items-center flex-grow space-x-4">
        ...
        <a href="{{ route('employees.create') }}" class="btn btn-primary bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 ml-4 whitespace-nowrap">
            Create New Employee
        </a>
    ...
    <!-- Employees Table -->
    <div id="employees-table" class="bg-white shadow-md rounded-lg overflow-hidden">
...
document.getElementById('btnCreate').addEventListener('click', function () {
    ...
    function fetchEmployees(searchQuery) {
        ...
        fetch(`{{ route('employees.index') }}?search=${searchQuery}`)
            .then(response => response.text())
            .then(data => {
                // Replace the content of the employee table with the new data
                document.getElementById('employees-table').innerHTML = data;
            })
            .catch(error => {
