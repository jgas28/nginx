...
<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <div class="overflow-hidden rounded-[30px] border border-slate-200/80 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.07)]">
        <div class="border-b border-slate-200/80 bg-gradient-to-r from-slate-50 via-white to-blue-50/70 px-6 py-7 sm:px-8">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                <div class="space-y-4">
                    <div class="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700">
                        ...
                        Running Balance - Collected
                    </div>
                    <div class="space-y-2">
                        <h1 class="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">Collected Funds Tracker</h1>
                        <p class="max-w-3xl text-[15px] leading-7 text-slate-600 sm:text-base">
                            Review all collected running balance movements with faster filters, searchable records, and a lighter transaction table.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="px-6 py-6 sm:px-8">
            <form id="running-balance-filter-form" method="GET" action="{{ route('running_balance.collectedFunds') }}" data-skip-inline-filter-enhancer class="rounded-[28px] border border-slate-200 bg-slate-50/80 p-5 shadow-inner shadow-slate-100">
        ...
        try {
            ...
            results.innerHTML = `
                <div class="rounded-[28px] border border-rose-200 bg-rose-50 px-5 py-4 text-[15px] text-rose-700">
                    Unable to load collected running balance right now. Please try again.
                </div>
            `;
        } finally {
