...
<div class="max-w-4xl mx-auto bg-white p-10 mt-10 rounded-2xl shadow-lg">
    ...
    <form action="{{ route('fleetCards.store') }}" method="POST" class="space-y-6">
        @csrf

        <div>
            <label for="account" class="block text-sm font-medium text-gray-700">Account</label>
            <input type="text" name="account" id="account" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
        </div>

        <div>
            <label for="account_name" class="block text-sm font-medium text-gray-700">Account Name</label>
            <input type="text" name="account_name" id="account_name" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
        </div>
