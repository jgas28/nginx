...
@section('content')
    <!-- Page Header -->
    <h1 class="text-2xl font-semibold mb-4 text-gray-700">Truck</h1>

    <!-- Container to hold both elements in a row -->
    <div class="flex justify-between items-center mb-4">
        <!-- Search Form (aligned to the left) -->
        <form method="GET" action="{{ route('trucks.index') }}" class="flex items-center flex-grow space-x-4">
        ...
        <a href="{{ route('trucks.create') }}" class="btn btn-primary bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 ml-4 whitespace-nowrap">
            Create New Truck
        </a>
    ...
    <div id="trucks-table" class="bg-white shadow-md rounded-lg overflow-hidden">
        @include('trucks.table', ['trucks' => $trucks])
    </div>
...
document.getElementById('btnCreate').addEventListener('click', function () {
    ...
    function fetchTrucks(searchQuery) {
        ...
        fetch(`{{ route('trucks.index') }}?search=${searchQuery}`)
