    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                <span>Show</span>
                <select id="trucks-per-page" class="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
        ...
        <div class="relative w-full lg:max-w-sm">
            ...
            <input
                ...
                id="trucks-search"
                value="{{ $search ?? '' }}"
                placeholder="Search code, truck, plate, or type..."
    ...
    <table class="min-w-full text-sm text-left text-slate-700">
        <thead class="bg-slate-50 text-xs uppercase tracking-[0.12em] text-slate-500">
            <tr>
                ...
                <th class="px-4 py-3 font-semibold sm:px-6">Truck Name</th>
                <th class="px-4 py-3 font-semibold sm:px-6">Plate Number</th>
        ...
        <tbody class="divide-y divide-slate-200 bg-white">
            @forelse($trucks as $truck)
                <tr class="hover:bg-slate-50/80">
                    ...
                    <td class="px-4 py-3.5 text-slate-600 sm:px-6">{{ $truck->plate_no }}</td>
                    <td class="px-4 py-3.5 text-slate-600 sm:px-6">
                        <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">
                            <i class="fas fa-road text-[10px] text-cyan-600"></i>
                            {{ $truck->truck_type }}
                        </span>
                    </td>
                    <td class="px-4 py-3.5 sm:px-6">
                        <div class="flex flex-wrap items-center justify-end gap-2">
                            <a href="{{ route('trucks.edit', $truck) }}" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-amber-200 bg-gradient-to-r from-amber-50 to-yellow-50 px-3 py-1.5 text-xs font-semibold text-amber-700 shadow-sm transition hover:-translate-y-0.5 hover:from-amber-100 hover:to-yellow-100 hover:shadow">
                                <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-white/90 text-amber-600">
                                    <i class="fas fa-pen text-[10px]"></i>
                                </span>
                                Edit
                            </a>
                            <form action="{{ route('trucks.destroy', $truck) }}" method="POST" class="truck-delete-form">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-12 text-center text-slate-500 sm:px-6">
                        ...
                        <p class="mt-3 font-medium text-slate-700">No trucks found</p>
                        <p class="mt-1 text-sm">Try a different search term or add a new truck.</p>
                    </td>
                </tr>
            @endforelse
        </tbody>
...
<div class="flex flex-col gap-3 border-t border-slate-200 px-4 py-4 text-sm text-slate-500 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
    <p>
        Showing
        <span class="font-semibold text-slate-700">{{ $trucks->firstItem() ?? 0 }}</span>
        to
        <span class="font-semibold text-slate-700">{{ $trucks->lastItem() ?? 0 }}</span>
        of
        <span class="font-semibold text-slate-700">{{ $trucks->total() }}</span>
        entries
    </p>
    <div class="trucks-pagination">
        {{ $trucks->links('pagination::tailwind') }}
    </div>
</div>
