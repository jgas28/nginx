...
<div class="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <div class="inline-flex items-center gap-2 rounded-full bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 ring-1 ring-blue-100">
                    ...
                    Liquidation Report - Admin
                </div>
                <h1 class="mt-4 text-3xl font-bold tracking-tight text-slate-900">Admin Liquidation Report</h1>
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                    Review admin and RPM liquidation records with supplier filtering, payment names, and reference numbers in one report table.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('liquidations.liquidationReportAdmin') }}" method="GET" id="liquidation-report-admin-filter-form" class="flex flex-col items-stretch justify-start gap-4 md:flex-row md:items-end md:justify-between">
            <div class="min-w-0 w-full flex-1 md:max-w-[420px]">
                <label for="report_admin_supplier_id" class="sr-only">Supplier</label>
                <select
                    name="supplier_id"
                    id="report_admin_supplier_id"
            ...
            <div class="flex w-full shrink-0 flex-col items-stretch justify-start gap-3 sm:flex-row sm:items-end sm:justify-end md:w-auto">
                <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700 sm:w-auto">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/15">
                        <i class="fas fa-magnifying-glass text-xs"></i>
                    </span>
                    Filter Results
                </button>
                <a href="{{ route('liquidations.liquidationReportAdmin') }}" id="liquidation-report-admin-filter-reset" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-400 hover:text-slate-900 sm:w-auto">
    ...
    <div class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
        <div id="liquidation-report-admin-table"
             data-fast-table
             data-endpoint="{{ route('liquidations.liquidationReportAdmin', ['supplier_id' => request('supplier_id')]) }}"
             data-base-endpoint="{{ route('liquidations.liquidationReportAdmin') }}"
             data-search-selector="#liquidation-report-admin-search"
             data-per-page-selector="#liquidation-report-admin-per-page"
             data-pagination-selector=".liquidation-report-admin-pagination a">
            @include('liquidations.partials.liquidationReportAdmin-table', ['data' => $data, 'search' => $search, 'perPage' => $perPage])
        </div>
    </div>
</div>
...
<script>
    (() => {
        const form = document.getElementById('liquidation-report-admin-filter-form');
        const select = document.getElementById('report_admin_supplier_id');
        const table = document.getElementById('liquidation-report-admin-table');
        const resetLink = document.getElementById('liquidation-report-admin-filter-reset');
        ...
        async function loadFilteredTable() {
            ...
            try {
                ...
                console.error('Error loading admin liquidation report:', error);
            } finally {
