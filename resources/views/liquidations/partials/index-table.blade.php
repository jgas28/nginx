            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-700">{{ trim($requestorName) !== '' ? $requestorName : 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Date Created</p>
                        <p class="mt-1 text-sm text-slate-500">{{ optional($voucher->created_at)->format('Y-m-d') ?? 'N/A' }}</p>
                    </div>
                </div>
                <div class="flex flex-col gap-2 sm:flex-row">
                    <a href="{{ route('liquidations.liquidate', $item->id) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3.5 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-700 sm:w-auto">
                        <i class="fas fa-file-circle-check text-[11px]"></i>
                        Liquidate
                    </a>
                    <a href="{{ route('liquidations.edit', $item->id) }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-3.5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 sm:w-auto">
