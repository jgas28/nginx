    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                <span>Show</span>
                <select id="liquidation-report-admin-per-page" class="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
        ...
        <div class="relative w-full lg:max-w-sm">
            ...
            <input
                type="text"
                id="liquidation-report-admin-search"
                value="{{ $search ?? '' }}"
                placeholder="Search CVR, company, supplier, payment, reference..."
...
<div class="space-y-3 p-4 md:hidden">
    @forelse ($data as $approval)
        @php
            ...
            $paymentName = trim((string) ($approval->payment_name ?? ''));
            $referenceNumber = trim((string) ($approval->reference_number ?? ''));
            $displayNumber = '-';
    ...
    @empty
        <div class="rounded-2xl border border-dashed border-slate-200 px-4 py-10 text-center text-sm text-slate-500">
            No admin liquidation report records found.
        </div>
    @endforelse
</div>
            ...
            @empty
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                        ...
                        <p class="mt-3 font-medium text-slate-700">No admin liquidation report records found</p>
...
<div class="flex flex-col gap-3 border-t border-slate-200 px-6 py-4 text-sm text-slate-500 lg:flex-row lg:items-center lg:justify-between">
    ...
    <div class="liquidation-report-admin-pagination">
