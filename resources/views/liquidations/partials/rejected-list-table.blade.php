<div class="space-y-3 p-4 md:hidden">
    @forelse ($liquidations as $item)
        @php
            $cashVoucher = $item->cashVoucher ?? null;
            $deliveryRequest = $cashVoucher?->deliveryRequest;
            $allocation = $item->allocations[0] ?? null;
            $truck = $allocation?->truck;
            $cvrNumber = $cashVoucher?->cvr_number ? preg_replace('/\/\d+$/', '', $cashVoucher->cvr_number) : 'N/A';
            $truckName = $truck?->truck_name ?? 'N/A';
            $companyCode = $deliveryRequest?->company?->company_code ?? 'N/A';
            $expenseCode = $deliveryRequest?->expenseType?->expense_code ?? 'N/A';
            $cvrType = $cashVoucher?->cvr_type ?? null;
            $allowedTypes = ['delivery', 'pullout', 'others', 'freight'];
        @endphp
        <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="space-y-3">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">CVR Number</p>
                    <p class="mt-1 break-words text-sm font-semibold text-slate-900">
                        @if(in_array($cvrType, $allowedTypes))
                            {{ $cvrNumber }}-{{ $truckName }}-{{ $companyCode }}{{ $expenseCode }}
                        @elseif($cvrType === 'admin')
                            {{ $cvrNumber }}-{{ $cashVoucher->company->company_code }}{{ $cashVoucher->expenseTypes->expense_code }}
                        @elseif($cvrType === 'rpm')
                            {{ $cvrNumber }}-{{ $cashVoucher->trucks?->truck_name }}-{{ $cashVoucher->company->company_code }}{{ $cashVoucher->expenseTypes->expense_code }}
                        @else
                            {{ $cvrNumber }}
                        @endif
                    </p>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Amount</p>
                        <p class="mt-1 text-sm font-semibold text-emerald-600">PHP {{ number_format($item->total_expense, 2) }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Company</p>
                        <span class="mt-1 inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                            @if(in_array($cvrType, $allowedTypes))
                                {{ $companyCode }}
                            @else
                                {{ $cashVoucher?->company?->company_code ?? 'N/A' }}
                            @endif
                        </span>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">CV Type</p>
                        <p class="mt-1 text-sm text-slate-700">{{ $cvrType ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Date Created</p>
                        <p class="mt-1 text-sm text-slate-500">{{ \Carbon\Carbon::parse($item->created_at)->format('Y-m-d') }}</p>
                    </div>
                </div>
                <a href="{{ route('liquidations.rejectEdit', $item->id) }}"
                    class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-3.5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700">
                    <i class="fas fa-pen-to-square text-[11px]"></i>
                    Edit
                </a>
            </div>
        </article>
    @empty
        <div class="rounded-2xl border border-dashed border-slate-200 px-4 py-10 text-center text-sm text-slate-500">
            No rejected liquidations found.
        </div>
    @endforelse
</div>

<div class="hidden overflow-x-auto md:block">
    <table class="min-w-full divide-y divide-slate-200 text-sm text-slate-700">
        <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
            <tr>
                <th class="px-6 py-4">CVR Number</th>
                <th class="px-6 py-4">Amount</th>
                <th class="px-6 py-4">Company Code</th>
                <th class="px-6 py-4">CV Type</th>
                <th class="px-6 py-4">Date Created</th>
                <th class="px-6 py-4 text-right">Actions</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-slate-200 bg-white">
            @forelse ($liquidations as $item)
                @php
                    $cashVoucher = $item->cashVoucher ?? null;
                    $deliveryRequest = $cashVoucher?->deliveryRequest;
                    $allocation = $item->allocations[0] ?? null;
                    $truck = $allocation?->truck;
                    $cvrNumber = $cashVoucher?->cvr_number ? preg_replace('/\/\d+$/', '', $cashVoucher->cvr_number) : 'N/A';
                    $truckName = $truck?->truck_name ?? 'N/A';
                    $companyCode = $deliveryRequest?->company?->company_code ?? 'N/A';
                    $expenseCode = $deliveryRequest?->expenseType?->expense_code ?? 'N/A';
                    $cvrType = $cashVoucher?->cvr_type ?? null;
                    $allowedTypes = ['delivery', 'pullout', 'others', 'freight'];
                @endphp
                <tr class="transition hover:bg-slate-50/80">
                    <td class="px-6 py-4 font-semibold text-slate-900">
                        <div class="inline-flex items-center gap-3">
                            <span class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-rose-50 text-rose-600">
                                <i class="fas fa-file-circle-xmark text-sm"></i>
                            </span>
                            <span>
                                @if(in_array($cvrType, $allowedTypes))
                                    {{ $cvrNumber }}-{{ $truckName }}-{{ $companyCode }}{{ $expenseCode }}
                                @elseif($cvrType === 'admin')
                                    {{ $cvrNumber }}-{{ $cashVoucher->company->company_code }}{{ $cashVoucher->expenseTypes->expense_code }}
                                @elseif($cvrType === 'rpm')
                                    {{ $cvrNumber }}-{{ $cashVoucher->trucks?->truck_name }}-{{ $cashVoucher->company->company_code }}{{ $cashVoucher->expenseTypes->expense_code }}
                                @else
                                    {{ $cvrNumber }}
                                @endif
                            </span>
                        </div>
                    </td>
                    <td class="px-6 py-4 font-semibold text-emerald-600">
                        PHP {{ number_format($item->total_expense, 2) }}
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">
                            <i class="fas fa-building text-[10px] text-blue-500"></i>
                            @if(in_array($cvrType, $allowedTypes))
                                {{ $companyCode }}
                            @else
                                {{ $cashVoucher?->company?->company_code ?? 'N/A' }}
                            @endif
                        </span>
                    </td>
                    <td class="px-6 py-4 capitalize text-slate-700">{{ $cvrType ?? 'N/A' }}</td>
                    <td class="px-6 py-4 text-slate-500">{{ \Carbon\Carbon::parse($item->created_at)->format('Y-m-d') }}</td>
                    <td class="px-6 py-4">
                        <div class="flex justify-end">
                            <a href="{{ route('liquidations.rejectEdit', $item->id) }}"
                                class="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-3.5 py-2 text-xs font-semibold text-white transition hover:bg-blue-700">
                                <i class="fas fa-pen-to-square text-[11px]"></i>
                                Edit
                            </a>
                        </div>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                        <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-slate-100 text-slate-400">
                            <i class="fas fa-folder-open text-lg"></i>
                        </div>
                        <p class="mt-3 font-medium text-slate-700">No rejected liquidations found</p>
                        <p class="mt-1 text-sm">Try a different search term.</p>
                    </td>
                </tr>
            @endforelse
        </tbody>
    </table>
</div>

<div class="flex flex-col gap-3 border-t border-slate-200 px-6 py-4 text-sm text-slate-500 lg:flex-row lg:items-center lg:justify-between">
    <p>
        Showing
        <span class="font-semibold text-slate-700">{{ $liquidations->firstItem() ?? 0 }}</span>
        to
        <span class="font-semibold text-slate-700">{{ $liquidations->lastItem() ?? 0 }}</span>
        of
        <span class="font-semibold text-slate-700">{{ $liquidations->total() }}</span>
        entries
    </p>
    <div class="rejected-list-pagination">
        {{ $liquidations->links('pagination::tailwind') }}
    </div>
</div>
