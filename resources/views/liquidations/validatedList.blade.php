...
@section('title', 'Liquidation Collection Queue')
...
<div class="mx-auto max-w-7xl space-y-6 py-8">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <div class="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 ring-1 ring-emerald-100">
                    ...
                    Validated Liquidations
                </div>
                <h1 class="mt-4 text-3xl font-bold tracking-tight text-slate-900">Collection Queue</h1>
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                    Track validated liquidations with a faster searchable queue, balanced filters, and clearer collection actions.
                </p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('liquidations.validatedList') }}" method="GET" id="liquidations-validated-filter-form" class="flex flex-col items-stretch justify-start gap-4 md:flex-row md:items-end md:justify-between">
            <div class="min-w-0 w-full flex-1 md:max-w-[420px]">
                ...
                <select
                    ...
                    class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                >
    ...
    <div id="liquidations-validated-table" class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">

        {{-- Static toolbar --}}
        <div class="border-b border-slate-200 px-6 py-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div class="relative w-full lg:max-w-sm">
                    <div class="pointer-events-none absolute left-3 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-emerald-50 text-emerald-600 shadow-sm">
                        <i class="fas fa-search text-sm"></i>
                    </div>
                    <input type="text" id="liquidations-validated-search" value="{{ $search ?? '' }}"
                        placeholder="Search CVR, company, supplier, collector, expense..."
                        autocomplete="off"
                        class="w-full rounded-xl border border-slate-300 bg-slate-50 py-2.5 pl-14 pr-4 text-sm text-slate-900 outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-100">
                </div>
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        <span>Show</span>
                        <select id="liquidations-validated-per-page" class="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                    ...
                    <div id="liquidations-validated-count" class="text-sm text-slate-500">
        ...
        {{-- Data area --}}
        <div class="relative">
            <div id="liquidations-validated-loading" class="pointer-events-none absolute inset-x-0 top-10 z-20 hidden justify-center">
            ...
            <div id="liquidations-validated-data">
                @include('liquidations.partials.validated-list-table', ['liquidations' => $liquidations, 'perPage' => $perPage, 'overview' => $overview])
            </div>
        </div>
...
<script>
    (() => {
        const form = document.getElementById('liquidations-validated-filter-form');
        const select = document.getElementById('cvr_type');
        const dataContainer = document.getElementById('liquidations-validated-data');
        const countEl = document.getElementById('liquidations-validated-count');
        const loadingEl = document.getElementById('liquidations-validated-loading');
        const resetLink = document.getElementById('liquidations-validated-filter-reset');

        if (!form || !select || !dataContainer) return;

        const baseEndpoint = '{{ route('liquidations.validatedList') }}';
        let state = {
        ...
        async function loadTableData({ cvrType, search, perPage, url } = {}) {
            ...
            try {
                ...
                console.error('Error loading validated liquidation list:', error);
            } finally {
        ...
        wrapper.innerHTML = `
            <button type="button" class="flex w-full items-center gap-3 rounded-2xl border border-slate-300 bg-white px-3 py-3 text-left text-sm text-slate-700 shadow-sm transition hover:border-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><i class="fas fa-layer-group text-sm"></i></span>
            ...
            <div class="searchable-select-panel absolute left-0 right-0 z-30 mt-2 hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/10" style="max-width:420px;">
                <div class="border-b border-slate-200 p-3"><div class="relative">
                    ...
                    <input type="text" placeholder="Search voucher type..." class="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm text-slate-700 outline-none transition focus:border-emerald-300 focus:bg-white focus:ring-2 focus:ring-emerald-100" data-select-search>
                </div></div>
        ...
        function renderOptions(term = '') {
            ...
            Array.from(select.options).forEach((option) => {
                ...
                btn.className = `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${option.selected ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700 hover:bg-slate-100'}`;
                btn.innerHTML = `<span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${option.selected ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}"><i class="fas fa-layer-group text-xs"></i></span><span class="min-w-0 flex-1 truncate">${option.textContent.trim()}</span>${option.selected ? '<i class="fas fa-check text-xs text-emerald-500"></i>' : ''}`;
