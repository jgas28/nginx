...
<div class="mx-auto max-w-7xl space-y-6 py-8">
    ...
    <div
        id="liquidations-validated-table"
        class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm"
        data-fast-table
        data-endpoint="{{ route('liquidations.validatedList', array_filter(['cvr_type' => $cvrType], fn ($value) => $value !== null && $value !== '')) }}"
        data-base-endpoint="{{ route('liquidations.validatedList') }}"
        data-search-selector="#liquidations-validated-search"
        data-per-page-selector="#liquidations-validated-per-page"
        data-pagination-selector=".liquidations-validated-pagination a"
    >
        @include('liquidations.partials.validated-list-table', [
...
<script>
    (() => {
        ...
        const table = document.getElementById('liquidations-validated-table');
        ...
        function renderOptions(term = '') {
            ...
            Array.from(select.options).forEach((option) => {
                ...
                button.type = 'button';
                button.className = `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${option.selected ? 'bg-emerald-50 text-emerald-700' : 'text-slate-700 hover:bg-slate-100'}`;
                button.innerHTML = `
                    <span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${option.selected ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}">
                        <i class="fas fa-layer-group text-xs"></i>
                    </span>
                    <span class="min-w-0 flex-1 truncate">${option.textContent.trim()}</span>
                    ${option.selected ? '<i class="fas fa-check text-xs text-emerald-500"></i>' : ''}
                `;
            ...
            emptyState.classList.toggle('hidden', visibleCount === 0 ? false : true);
        }
