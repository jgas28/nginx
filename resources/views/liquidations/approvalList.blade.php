...
<div class="mx-auto max-w-7xl space-y-6 py-8">
    ...
    <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('liquidations.approvalList') }}" method="GET" id="liquidations-approval-filter-form" class="flex items-end justify-between gap-4">
            ...
            <div class="flex shrink-0 items-end justify-end gap-3 whitespace-nowrap">
                <button type="submit" class="inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-violet-700">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/15">
                        <i class="fas fa-filter text-xs"></i>
                    </span>
                    Filter Results
                </button>
                <a href="{{ route('liquidations.approvalList') }}" id="liquidations-approval-filter-reset" class="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-400 hover:text-slate-900">
    ...
    <div
        id="liquidations-approval-table"
        class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm"
        data-fast-table
        data-endpoint="{{ route('liquidations.approvalList', array_filter(['cvr_type' => $cvrType], fn ($value) => $value !== null && $value !== '')) }}"
        data-base-endpoint="{{ route('liquidations.approvalList') }}"
        data-search-selector="#liquidations-approval-search"
        data-per-page-selector="#liquidations-approval-per-page"
        data-pagination-selector=".liquidations-approval-pagination a"
    >
        @include('liquidations.partials.approval-list-table', [
...
<script>
    (() => {
        ...
        const table = document.getElementById('liquidations-approval-table');
