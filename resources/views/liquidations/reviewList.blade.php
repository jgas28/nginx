...
<div class="mx-auto max-w-7xl space-y-6 py-8">
    ...
    <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('liquidations.reviewList') }}" method="GET" id="liquidations-review-filter-form" class="flex flex-col items-stretch justify-start gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div class="flex w-full shrink-0 flex-col items-stretch justify-start gap-3 whitespace-nowrap sm:flex-row sm:items-end sm:justify-end md:w-auto">
                <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-cyan-700 sm:w-auto">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/15">
                        <i class="fas fa-filter text-xs"></i>
                    </span>
                    Apply Filters
                </button>
                <a href="{{ route('liquidations.reviewList') }}" id="liquidations-review-filter-reset" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-400 hover:text-slate-900 sm:w-auto">
    ...
    <div id="liquidations-review-table" class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
        ...
        <div class="border-b border-slate-200 px-6 py-4">
            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div class="relative w-full lg:max-w-sm">
                    ...
                    <input type="text" id="liquidations-review-search" value="{{ $search ?? '' }}"
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        <span>Show</span>
                        <select id="liquidations-review-per-page" class="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-100">
                    ...
                    <div id="liquidations-review-count" class="text-sm text-slate-500">
        ...
        {{-- Data area --}}
        <div class="relative">
            <div id="liquidations-review-loading" class="pointer-events-none absolute inset-x-0 top-10 z-20 hidden justify-center">
                <div class="inline-flex items-center gap-2.5 rounded-full border border-slate-200 bg-white px-5 py-2.5 shadow-lg">
                    <svg class="h-4 w-4 animate-spin text-cyan-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <div id="liquidations-review-data">
                @include('liquidations.partials.review-list-table', ['liquidations' => $liquidations, 'perPage' => $perPage, 'overview' => $overview])
            </div>
        </div>
...
<script>
    (() => {
        ...
        const dataContainer = document.getElementById('liquidations-review-data');
        const countEl = document.getElementById('liquidations-review-count');
        const loadingEl = document.getElementById('liquidations-review-loading');
        const resetLink = document.getElementById('liquidations-review-filter-reset');

        if (!form || !select || !dataContainer) return;

        const baseEndpoint = '{{ route('liquidations.reviewList') }}';
        let state = {
        ...
        wrapper.innerHTML = `
            <button type="button" class="flex w-full items-center gap-3 rounded-2xl border border-slate-300 bg-white px-3 py-3 text-left text-sm text-slate-700 shadow-sm transition hover:border-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-500">
                <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-cyan-50 text-cyan-600"><i class="fas fa-layer-group text-sm"></i></span>
        ...
        function renderOptions(term = '') {
            ...
            Array.from(select.options).forEach((option) => {
                ...
                btn.className = `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${option.selected ? 'bg-cyan-50 text-cyan-700' : 'text-slate-700 hover:bg-slate-100'}`;
                btn.innerHTML = `<span class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${option.selected ? 'bg-cyan-100 text-cyan-600' : 'bg-slate-100 text-slate-500'}"><i class="fas fa-layer-group text-xs"></i></span><span class="min-w-0 flex-1 truncate">${option.textContent.trim()}</span>${option.selected ? '<i class="fas fa-check text-xs text-cyan-500"></i>' : ''}`;
