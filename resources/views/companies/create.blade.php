    ...
    <div class="mx-auto max-w-5xl px-4">
        <div class="mb-6 rounded-[28px] border border-white/70 bg-white/80 px-6 py-6 shadow-[0_18px_45px_rgba(15,23,42,0.08)] backdrop-blur sm:px-8">
            ...
            <div>
                ...
                <h1 class="mt-4 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Create Company</h1>
                <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Add a new company record with complete and accurate details. Keep the code, company name, and location aligned for easier searching later.</p>
            </div>
        ...
        <div class="rounded-[30px] border border-white/70 bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.08)] sm:p-8">
            ...
            <form action="{{ route('companies.store') }}" method="POST" class="space-y-8" novalidate>
                @csrf
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div class="rounded-3xl border border-slate-200 bg-slate-50/70 p-5">
                        ...
                        <input type="text" name="company_code" id="company_code" value="{{ old('company_code') }}" required maxlength="255" autocomplete="off" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-100">
                    </div>
                ...
                <div class="flex flex-col-reverse gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-500">Make sure the company code is unique and easy to recognize in list pages.</p>
