<div class="border-b border-slate-200 px-4 py-4 sm:px-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between" data-fast-table-toolbar>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                <span>Show</span>
                <select data-fast-table-per-page class="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100">
            ...
            <div class="text-sm text-slate-500">
                {{ $companies->total() }} total companies
            </div>
        </div>
            ...
            @forelse($companies as $company)
                <tr class="hover:bg-slate-50/80">
                    <td class="px-4 py-3.5 text-sm text-slate-500 sm:px-6">
                        {{ ($companies->firstItem() ?? 0) + $loop->index }}
                    </td>
                    <td class="px-4 py-3.5 sm:px-6">
                        <span class="inline-flex items-center justify-center rounded-full bg-gradient-to-r from-sky-50 to-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 ring-1 ring-blue-100">
                    ...
                    <td class="px-4 py-3.5 sm:px-6">
                        <div class="flex flex-wrap items-center justify-end gap-2">
                            <a href="{{ route('companies.edit', $company) }}" class="inline-flex items-center justify-center gap-1.5 rounded-xl border border-amber-200 bg-gradient-to-r from-amber-50 to-yellow-50 px-3 py-1.5 text-xs font-semibold text-amber-700 shadow-sm transition hover:-translate-y-0.5 hover:from-amber-100 hover:to-yellow-100 hover:shadow">
                                <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-white/90 text-amber-600">
                                    <i class="fas fa-pen text-[10px]"></i>
                                </span>
                                Edit
                            </a>
                            <form action="{{ route('companies.destroy', $company) }}" method="POST" class="company-delete-form">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-12 text-center text-slate-500 sm:px-6">
                        ...
                        <p class="mt-3 font-medium text-slate-700">No companies found</p>
                        <p class="mt-1 text-sm">Try a different search term or add a new company.</p>
                    </td>
                </tr>
            @endforelse
        </tbody>
...
<div class="flex flex-col gap-3 border-t border-slate-200 px-4 py-4 text-sm text-slate-500 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
    <p>
        Showing
        <span class="font-semibold text-slate-700">{{ $companies->firstItem() ?? 0 }}</span>
        to
        <span class="font-semibold text-slate-700">{{ $companies->lastItem() ?? 0 }}</span>
        of
        <span class="font-semibold text-slate-700">{{ $companies->total() }}</span>
        entries
    </p>
    <div data-fast-table-pagination>
