    ...
    <tbody>
        @foreach($taxes as $tax)
            <tr class="hover:bg-gray-50">
                <td class="py-2 px-4 border-b">{{ $tax->description }}</td>
                <td class="py-2 px-4 border-b">{{ $tax->percentage }}</td>
                <td class="py-2 px-4 border-b">
                    <a href="{{ route('taxes.edit', $tax) }}" class="btn btn-warning bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600">
                        Edit
                    </a>
