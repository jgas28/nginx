...
<table class="table-auto w-full text-sm text-left text-gray-600">
    ...
    @foreach($deliveryRequests as $deliveryRequest)
        <tr class="hover:bg-gray-50">
            ...
            <!-- Actions (if needed) -->
            <td class="py-2 px-4 border-b">
                <a href="{{ route('deliveryRequest.edit', $deliveryRequest) }}" class="btn btn-warning bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600" title="Edit this delivery request">
                    Edit
                </a>
                @if($deliveryRequest->delivery_type  != 'Regular')
                <a href="{{ route('deliveryRequest.splitView', $deliveryRequest) }}" class="btn btn-success bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600" title="Edit this delivery request">
                    Split
                </a>
                @endif
                <form action="{{ route('deliveryRequest.destroy', $deliveryRequest) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this delivery request?');">
                    @csrf
                    @method('DELETE')
