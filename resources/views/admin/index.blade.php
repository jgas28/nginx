...
@section('content')
    <!-- Page Header -->
    <h1 class="text-2xl font-semibold mb-4 text-gray-700">Admin Cash Voucher Request</h1>

    <!-- Container to hold both elements in a row -->
    <div class="flex justify-between items-center mb-4">
        <!-- Search Form (aligned to the left) -->
        <form method="GET" action="{{ route('admin.index') }}" class="flex items-center flex-grow space-x-4">
        ...
        <a href="{{ route('admin.create') }}" class="btn btn-primary bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 ml-4 whitespace-nowrap">
            Create New Admin Cash Voucher
        </a>
    ...
    <div id="delivery-requests-table" class="bg-white shadow-md rounded-lg overflow-hidden">
        @include('admin.table', ['cashVouchers' => $cashVouchers])
    </div>
...
document.getElementById('btnCreate').addEventListener('click', function () {
    ...
    function fetchDeliveryRequests(searchQuery) {
        ...
        fetch(`{{ route('admin.index') }}?search=${searchQuery}`)
            .then(response => response.text())
            .then(data => {
                // Replace the content of the employee table with the new data
                document.getElementById('cashVouchers-table').innerHTML = data;
            })
            .catch(error => {
